...
class EnergyCalculator:
    ...
    def calc(self, config):
        ...
        logger.debug("EnergyCalculator.calc: e = {}, N = {}".format(e0, np.sum(state)))
        return e0
...
class LatticeGas(DFTLatticeGas):
    ...
    def trialstep(self, config, energy_now):
        logger.debug(">>> LatticeGas.trialstep: flipper")
        ...
        choice = random.randint(nsite)

        logger.debug("choice = {}, value = {}".format(choice, latgas_rep[choice][0]))

        # flip
        if latgas_rep[choice][0] == 'A':
            latgas_rep[choice][0] = '@'
        else:
...
def main(params):
    ...
    for p in ptable:
        ...
        calc_params = { 'Lx': Lx, 'Ly': Ly, 'J': J, 'mu': mu-Eads }
