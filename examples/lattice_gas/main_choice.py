...
class EnergyCalculator:
    ...
    def calc(self, config):
        ...
        state = np.array([(1 if v[0] == 'A' else 0)
                          for v in config.defect_sublattices[0].latgas_rep]).reshape(self.Lx,self.Ly)
...
class LatticeGas(DFTLatticeGas):
    ...
    def trialstep(self, config, energy_now):
        logger.debug(">>> LatticeGas.trialstep: chooser")
        ...
        sublat_bak = copy.deepcopy(config.defect_sublattices)

        latgas_rep = config.defect_sublattices[0].latgas_rep
        nsite = len(latgas_rep)

        while True:
            if random.rand() >= 0.5:
                logger.debug("try add")
                vac_list = [ i for i in range(nsite) if latgas_rep[i][0] == '@' ]
                if len(vac_list) == 0:
                    logger.debug("--- vac empty. retry")
                    continue
                else:
                    choice = random.choice(vac_list)
                    logger.debug("+++ add choice={}".format(choice))
                    latgas_rep[choice][0] = 'A'
                    break
            else:
                logger.debug("try remove")
                elem_list = [ i for i in range(nsite) if latgas_rep[i][0] == 'A' ]
                if len(elem_list) == 0:
                    logger.debug("--- elements empty. retry")
                    continue
                else:
                    choice = random.choice(elem_list)
                    logger.debug("+++ remove choice={}".format(choice))
                    latgas_rep[choice][0] = '@'
                    break
            
        energy_new = self.energy(config)

        # restore
        sublat = config.defect_sublattices
        config.defect_sublattices = sublat_bak

        dconfig = (sublat, )
        dE = energy_new - energy_now

        return dconfig, dE

    def newconfig(self, config, dconfig):
        sublat, = dconfig
        config.defect_sublattices = copy.deepcopy(sublat)
        return config
    ...
    def density(self, config):
        state = np.array([(1 if v[0] == 'A' else 0)
                          for v in config.defect_sublattices[0].latgas_rep])
        num = np.sum(state)
        return num
...
def main(params):
    ...
    for p in ptable:
        ...
        model = LatticeGas(energy_calculator,
                           enable_grandcanonical = True,
                           gc_ratio = 1.0)
