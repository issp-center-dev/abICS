...
class SevennetSolver(SolverBase):
    ...
    def __init__(self, ignore_species, use_pretrained, relax, fmax, device, pretrained):
        """
        Initialize the solver.

        """

        super(SevennetSolver, self).__init__("")
        self.path_to_solver = self.calculate_energy
        self.input = SevennetSolver.Input(ignore_species, use_pretrained, relax, fmax, device, pretrained)
        self.output = SevennetSolver.Output()
    ...
    class Input(object):
        ...
        def __init__(self, ignore_species=None, use_pretrained=False, relax=False, fmax=0.1, device="cpu", pretrained="7net-0"):
        ...
        def from_directory(self, base_input_dir):
            ...
            if not self.use_pretrained:
                self.calculator = SevenNetCalculator(os.path.join(base_input_dir, "deployed.pth"), device=self.device)
            else:
                self.calculator = SevenNetCalculator(model=self.pretrained, device=self.device)
    ...
    def create(cls, params: ALParams | DFTParams):
        ...
        return cls(ignore_species, use_pretrained, relax, fmax, device, pretrained)
