...
class aenet_trainer:
    def __init__(self, structures, energies, generate_inputdir, train_inputdir, predict_inputdir, 
                    generate_exe, train_exe):
    ...
    def prepare(self):
        ...
        with open("generate.in.head", "r") as fi:
            ...
            xsf_paths = [os.path.join(xsfdir, "structure.{}.xsf".format(i)) for i in range(self.numdata)]
        ...
        with open(os.path.join(os.getcwd(), "stdout"), "w") as fi:
            subprocess.run(command, shell=True, stdout=fi, stderr=subprocess.STDOUT, check=True)
    ...
    def train(self):
        ...
        os.chdir("train")
        shutil.copyfile(os.path.join(self.generate_outputdir, "aenet.train"), 
    ...
    def new_baseinput(self, baseinput_dir):
        ...
        shutil.copyfile(os.path.join(self.predict_inputdir, "predict.in"), 
                        os.path.join(baseinput_dir, "predict.in"))
        NNPid_str = "{:05d}".format(self.train_minID)
        NNPfiles = [fi for fi in os.listdir(self.train_outputdir) 
                        if NNPid_str in fi]
        for fi in NNPfiles:
            shutil.copyfile(os.path.join(self.train_outputdir, fi),
