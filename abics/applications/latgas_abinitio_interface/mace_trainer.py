...
class MaceTrainer(TrainerBase):
    def __init__(
        ...
        self.train_exe.append("--config=input.yaml")
    ...
    def prepare(self, latgas_mode = True, st_dir = "maceXSF"):
        rootdir = os.getcwd()
    ...
    def generate_run(self, xsfdir="maceXSF", generate_dir="generate"):
        # prepare generate
        ...
        for ase_ in ases:
            ase_.info["energy_mace"] = ase_.get_potential_energy()
    ...
    def new_baseinput(self, baseinput_dir, train_dir = "train"):
        ...
        os.chdir(train_dir)
        with open("input.yaml", "r") as yml:
            yaml_dic = yaml.safe_load(yml)
        runname = yaml_dic["name"]
        shutil.copy("{}.model".format(runname),os.path.join(baseinput,"deployed.model"))
        os.chdir(pathlib.Path(os.getcwd()).parent)
