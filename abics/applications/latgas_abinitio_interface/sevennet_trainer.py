...
#
# abICS wrapper of SevenNet solver
...
class SevennetTrainer(TrainerBase):
    def __init__(
        self,
    ...
    def prepare(self, latgas_mode = True, st_dir = "sevennetXSF"):
        rootdir = os.getcwd()
    ...
    def generate_run(self, xsfdir="sevennetXSF", generate_dir="generate"):
        # prepare generate
    ...
    def new_baseinput(self, baseinput_dir, train_dir = "train"):
        ...
        checkpoint_nums = [int(f.replace("checkpoint_", "").replace(".pth", "")) for f in checkpoint_files]
        max_checkpoint_num = max(checkpoint_nums)
        checkpoint_file = f"checkpoint_{max_checkpoint_num}.pth"
        #subprocess.run(["sevenn_get_model", checkpoint_file], check=True)
        #shutil.copy("deployed_serial.pt", os.path.join(baseinput, "deployed.pt"))
        shutil.copy(checkpoint_file, os.path.join(baseinput, "deployed.pth"))
        os.chdir(pathlib.Path(os.getcwd()).parent)
